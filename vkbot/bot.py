...
def main():
    ...
    # ─── Устанавливаем StateDispenser для FSM ───
    state_dispenser = bot.state_dispenser

    # ─── Регистрируем blueprint с обработчиками ───
    booking_bp.load(bot)

    # ─── Предзагрузка кэша YClients ───
    from .cache import get_cache
    cache = get_cache(API_BASE)
    logger.info("Предзагрузка кэша YClients...")
    cache.refresh_cache()
    logger.info(
        f"Загружено: {len(cache.categories)} категорий, "
