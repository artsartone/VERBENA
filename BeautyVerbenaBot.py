...
_YC_CACHE_LOCK = asyncio.Lock()  # блокировка для предотвращения одновременных refresh


def _is_cache_fresh() -> bool:
    """Проверить, не устарел ли кэш YClients."""
    global _YC_CACHE_TIMESTAMP
    return (datetime.now() - _YC_CACHE_TIMESTAMP).total_seconds() < _YC_CACHE_TTL
...
async def show_career(update: Update,
    ...
    text = ("🤝 <b>Хотите присоединиться к команде VERBENA?</b>\n\n"
            ...
            "Напишите нам в одном сообщении:\n"
            "• Ваше имя\n"
            "• Телефон\n"
            "• Опыт работы\n"
        ...
        if resp.status_code == 201:
            ...
            if client_id:
                try:
                    ...
                    already_linked = status_resp.ok and status_resp.json().get(
                        "linked")
                except Exception:
            ...
            await query.edit_message_text(
                ...
                "<i>Вы можете посмотреть свои записи в любое время через главное меню.</i>",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(keyboard),
            )
...
def main() -> None:
    ...
    refresh_yc_cache()
    logger.info(f"Загружено: {len(_YC_CATEGORIES_CACHE)} категорий, "
